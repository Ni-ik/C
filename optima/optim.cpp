...
//номера начала, конца, установленная мощность тр-ра
float Kzagr[i],Cos[i],Tnb[i];
//коэф. загрузки, мощности, время исп. наиб. нагрузки
int NK[i],NKL[i];//номер трансформатора и линии в каталогах
int KAT,KAL,NU;
...
dWPxx[j]=T*Pxx[j]*Usr[Nu[j]]*Usr[Nu[j]]/(Unom*Unom);//Потери акт.хх в тыс.кВт*ч
dWQxx[j]=T*Qxx[j]*Usr[Nu[j]]*Usr[Nu[j]]/(Unom*Unom);//Потери реакт.хх в тыс. кВт*ч
dWPn[j]=(WPku[j]*WPku[j]+WQku[j]*WQku[j])*RU[j]*KforP[j]/
(Usr[Ku[j]]*Usr[Ku[j]]*T*1000);//Потери акт.нагрузочные в тыс.кВт*ч
dWQn[j]=(WPku[j]*WPku[j]+WQku[j]*WQku[j])*XU[j]*KforQ[j]/
(Usr[Ku[j]]*Usr[Ku[j]]*T*1000);	//Потери реакт.нагрузочные в тыс. кВт*ч
...
printf("%f Wn=%f dW=%f dWk=%f\n",KforP[j],WPnu[j],dWPxx[j]+dWPn[j],WPku[j]);
}
...
for(k=1;k<=NU;k++)
{printf("%d-%d %f Wn=%f dW=%f dWk=%f\n",Nu[k],Ku[k], KforP[k],WPnu[k],dWPxx[k]+dWPn[k],WPku[k]);}
}

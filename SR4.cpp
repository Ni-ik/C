					 ...
					 )
{
	...
	case DLL_PROCESS_ATTACH:
		MessageBox(NULL, TEXT("Çàãðóæàåòñÿ áèáëèîòåêà Ïåòðàøåâè÷"), TEXT(""), MB_OK);
		break;
	...
	case DLL_PROCESS_DETACH:
			MessageBox(NULL, TEXT("Âûãðóæàåòñÿ áèáëèîòåêà Ïåòðàøåâè÷"), TEXT(""), MB_OK);
		break;
...
void loadFile(char * string, char * fileName)
{
		...
		DWORD BuffSize=GetFileSize(hFile, NULL);//Ðàçìåð ôàéëà
		//×òåíèå èç ôàéëà â áóôåð
		DWORD dwNumbOfBytes;
		if (BuffSize<MAX_BYTES)
		{
			ReadFile(hFile, string, BuffSize, &dwNumbOfBytes,NULL);
			string[BuffSize] = '\0';
		}
		else
		{
...
//äîáàâëÿåò â êîíåö ôàéëà fileName ñòðîêó string
void updateFile(char * fileName, char * string)
{
	if (fileName == NULL || string == NULL) //ïðîâåðêà âõîäíûõ ïàðàìåòðîâ
		return;

	FILE* file =  fopen(fileName,"ab"); //îòêðûâàåì ôàéë äëÿ äîáàâëåíèÿ

    if (file == NULL) { //ïðîâåðêà ÷òî ôàéë îòêðûò
		MessageBox(NULL,"Îøèáêà ïðè îòêðûòèè ôàéëà äëÿ çàïèñè","Îøèáêà",MB_OK);
        return;
    }
    
    fwrite(string, 1, strlen(string), file); //äîáàâëÿåì â ôàéë ñòðîêó
